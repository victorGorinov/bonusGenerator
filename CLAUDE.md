...
| POST | `/api/campaign/description` | 15/min | DescriptionSchema | `createCampaignController().description` — customer-facing offer-page copy (AI prose) + deterministic terms from config |
...
| POST | `/api/tournament/description` | 15/min | TournamentDescriptionSchema | `createTournamentController().description` — offer-page copy + deterministic terms |
...
| POST | `/api/loyalty/description` | 15/min | LoyaltyDescriptionSchema | `createLoyaltyController().description` — program-page copy + deterministic terms |
...
## Abuse-risk / anti-fraud audit (2026-07-26)

Deterministic, no-AI structural check that flags how exploitable a promo config is to bonus abuse — mirrors the Parameter-benchmarks explainability layer, not the AI-audit path. Answers "will hunters farm this?" from config params alone, so it's explainable and defensible to an operator.

**Shared shape** — `src/domain/shared/RiskVector.ts` (browser mirror `public/risk-vector.js`, parity-tested):
- `RiskVector { key, severity:'info'|'warn'|'high'|'critical', current?, threshold?, mitigationKey }`
- `RiskAssessment { score:0..100, level:'low'|'elevated'|'high'|'critical', vectors[] }`
- `scoreVectors(vectors)` — sums `SEVERITY_WEIGHT` (info 0 / warn 12 / high 30 / critical 55), caps at 100, sorts most-severe first. Level = cumulative thresholds (≥60 critical, ≥30 high, ≥12 elevated) **floored to the single most-severe vector** — so a lone `critical` (score 55, below the 60 cutoff) still reads as level `critical`, never `high`. **The floor must stay in both the TS and the mirror** or parity breaks.

**Three per-type engines** (pure; each `assess*Abuse()` returns a `RiskAssessment`; each has a `public/*-abuse.js` mirror + a `*.abuseRisk.test.js` core test + a `*.abuse.parity.test.js`):
- `src/domain/bonus/abuseRisk.ts` — anchor `wagerX vs econ.breakeven_wager`. Vectors: `ev_positive` (crit/high/`ev_thin_margin` warn by `wagerX/be` ratio; **skipped for sweeps** where `wagerX=0` is expected), `low_contrib_clearing` (low-variance games in `cfg.contrib` with pct ≥20/50), `over_generous`, `cheap_farm_mind` (bonusSize/minD ≥20), `no_maxwin` (info, warn if EV), `ndb_unbounded` / `ndb_no_kyc` (trigger ≠ `v_reg_verify`), `cashback_hedge`, `no_maxbet` (info). Reads a slice of the buildConfig output; `opts.activeTypes` gates NDB/cashback vectors.
- `src/domain/loyalty/abuseRisk.ts` — anchor `earnRateWager/redeemRate` vs `ASSUMED_HOUSE_EDGE` (0.03; loyalty carries no per-game RTP so it's assumed). Vectors: `ev_positive_loop`, `deposit_rebate_high` (earnRateDeposit/redeemRate ≥15%/25%), `redeem_no_wager` (one deposit-cycle clears `redeemMinPoints` — `current`=pts/cycle, `threshold`=min floor), `tier_gaming` (top-tier cashback ≥15%/20%), `mission_exploit` (repeatable cash-bonus missions), `points_no_expiry` (info).
- `src/domain/tournament/abuseRisk.ts` — anchor prize-EV per free entry + field size. Reads `(params, econ)`. Vectors: `ev_positive_freeroll` (freeroll/ticket entry; high if `econ.participantsMid` <300), `multiaccount_magnet` (freeroll + segment=all), `turnover_grind` (`most_spins` high / `total_wins` warn), `variance_abuse` (`highest_multiplier` + reentry≠single), `reentry_unbounded` (paid + unlimited).

**UI** — deterministic, no API call:
- **Configurator** (`configurator.js`): a **🛡️ Abuse risk** tab for all 3 types (added to each tab array + an `if (ai.tab==='abuse') return renderAbuseTabContent(type)` branch). `abuseAssess(type)` builds the input from CS state (bonus: `B.config` + `B.active` keys; tournament: `T.result.{params,econ}`; loyalty: `LY.result.config`) and calls the matching `window.Retomat{Bonus,Loyalty,Tournament}Abuse`. Renders a score circle + severity chips + current/threshold + mitigation + disclaimer. i18n `abuse_*` (tab/level/sev labels + `abuse_v_<vectorKey>` titles + `abuse_mit_*` mitigations) in `CFG_I18N` (EN+RU). Mirror script tags in `configurator.html`.
- **Campaign Generator** (`campaign-generator.js`): compact `#abuseCard` badge (`renderAbuseBadge`) — score + top-2 non-info vectors, own self-contained `_ABUSE_GEN_I18N` (kept out of the main `t()` dict). Reads `data.selectedMechanics` + `data.econ`/`contrib`/`r`/`requestedTypes` from the `/api/campaign/generate` response. Mirror script tags in `campaign-generator.html`.

**Scope note:** structural check on the **config**, NOT a live transaction/behavioural fraud monitor (said so in the panel disclaimer). Engines are client-side only — not imported by any route. Tests: 33 (core + parity).
...
- **Wheel of Fortune Phase 2** (first iteration + generator-hub tab shipped 2026-07-19/20, see Completed log): full segment editor (add/remove arbitrary segments, not just tweak the 3 presets' fixed segments); more prize types (physical goods with real fulfilment cost, tiered jackpots); server-side persistence for saved wheels (both the Configurator Save and the hub's `savedWheels` write to `cfgSaved`/localStorage + mirror as `configs` only — a dedicated `wheels` entity in the `SavedItems` whitelist + migration + `savedWheels` nav badge is still pending); per-preset regulatory auto-warnings as a passive UI banner in the Configurator (BR welcome-wheel prohibition is shown in the hub's region-recommendation note + AI audit, but not yet a passive benchmark-style banner in the Configurator wheel tab)
- **Abuse-risk Phase 2 (AI narrative layer)** — the deterministic engine (see "Abuse-risk / anti-fraud audit") stays the source of truth; add an optional AI layer *on top of* its findings, never replacing them. Scope: new `POST /api/{campaign,tournament,loyalty}/abuse-audit` (aiLimiter + existing `requireFeature`) that takes the `RiskAssessment` (score/level/vectors) + the config and returns (a) a prioritised, plain-language narrative of the worst vectors, (b) concrete tightened-parameter targets per vector (e.g. "raise wager 20×→26×", "cap NDB maxWin 10×") with the projected `score`/`level` after applying them — recomputed by the deterministic engine, not hallucinated, and (c) exploit-scenario prose ("how a hunter would farm this"). New `AbuseAuditResponseSchema` in `src/ai/parser.ts` + prompts `src/ai/prompts/{,tournament-,loyalty-}abuse-audit.prompt.ts` (feed the vectors as structured context so the model never re-derives the math — same discipline as the F1 optimize-prompt fix). Frontend: an "AI explain" button inside the existing 🛡️ Abuse risk tab (button-triggered, not auto — consistent with the Step-4 AI change). Keep it a **wheel-less, 4th-type-aware** design: wheel has an abuse surface too but no engine yet, so either add `src/domain/wheel/abuseRisk.ts` first or scope the AI layer to the 3 existing types. Deferred deliberately: the deterministic layer already answers "is this exploitable?"; the AI layer only adds prose + prioritisation + projected-fix deltas.
...
- ~~**Abuse-risk / anti-fraud audit (Bonus/Tournament/Loyalty)**~~ — done 2026-07-26, from expert feedback (a promo config should surface how farmable it is, not just its economics). Deterministic, no-AI structural engine mirroring the benchmarks-explainability layer: shared `RiskVector`/`scoreVectors` (`src/domain/shared/RiskVector.ts` + mirror `public/risk-vector.js`) with **level floored to the single most-severe vector** (a lone `critical` at score 55 no longer under-reports as `high`); three per-type engines `src/domain/{bonus,loyalty,tournament}/abuseRisk.ts` with browser mirrors `public/{bonus,loyalty,tournament}-abuse.js` (parity-tested). Anchors: bonus `wagerX vs breakeven_wager`, loyalty `earnRateWager/redeemRate vs 3% house edge`, tournament prize-EV per free entry + field size. UI: **🛡️ Abuse risk** tab in the unified Configurator for all 3 types (score + severity chips + mitigations, EN/RU in `CFG_I18N`) + a compact risk badge in the Campaign Generator (`renderAbuseBadge`, own `_ABUSE_GEN_I18N`). Client-side only (no route imports the engines). Tests: 33 new (core + parity), 820 total green; typecheck + lint clean. Reviewed (independent adversarial pass) → fixed 2 self-found defects before ship: the score→level under-report (above) and a `redeem_no_wager` current/threshold swap. Shipped to prod (`main` dcc2adc) + stage via surgical cherry-pick (branches had 3-way diverged). See **Abuse-risk / anti-fraud audit** section. Phase 2 (AI narrative on top of the deterministic findings) is planned — see Pending P2. **Same push also fixed the F1 optimize recommendation** inverting the wager↔breakeven comparison (`src/ai/prompts/optimize.prompt.ts`): the wager row now states the relation explicitly (server-computed BELOW/ABOVE via `wagerX < beW`) + a corrected F1 formula note + an anti-inversion instruction (RU+EN) — the model was writing "wager 20× exceeds breakeven 21.2×".
- ~~**Offer Description (customer-facing page copy + T&C) — all promo types**~~ — done 2026-07-21, repurposing the AI text feature toward acquisition/on-site copy (expert feedback: 5-channel CRM copy is undifferentiated; the moat is the structured config). New **hybrid** output type: a deterministic **terms block** (exact params rendered straight from the config — never the AI) + AI **prose** (title/hook/how-it-works/CTA) + AI **Terms & Conditions** (6–10 clauses, license-aware, anchored to the exact terms). Response reuses the generic `OfferDescriptionResponseSchema`/`parseDescriptionResponse` (`title,hook,howItWorks[],termsIntro,cta,termsAndConditions[]`); the use-case merges deterministic `terms[]` in. Per-type deterministic terms builders: `src/domain/{campaign/offerTerms.ts, tournament/offerTerms.ts, loyalty/offerTerms.ts, wheel/offerTerms.ts}` (all return `OfferTerm{label,value}`, EN/RU labels). Per-type prompts `src/ai/prompts/{description,tournament-description,loyalty-description,wheel-description}.prompt.ts` (2000-token budget). Routes `POST /api/{campaign,tournament,loyalty,wheel}/description` (aiLimiter + existing `requireFeature`). **Frontend:** Bonus — new 📄 Описание sub-tab in Step 4 (`campaign-generator.js` + hub `generator.js`); Tournament — Description button in Step 4 (`tournament-generator.js` + `generator-tournament.js`); Loyalty — Description AI-tab (`loyalty-generator.js` + `generator-loyalty.js`); Wheel — Description card in result view (`generator-wheel.js`). Each: generate-on-click, terms table + prose + T&C list, copy button (+ regen). The client render+copy is a single shared module `public/offer-desc.js` (`window.OfferDesc.render/plainText/copyText`, loaded on campaign-/generator/tournament-/loyalty-generator pages) — the copy button calls a named per-fork global (`copyTournDesc`/`tgCopyDesc`/`copyLoyDesc`/`lyCopyDesc`/`whCopyDesc`/`copyDescription`) that reads the fork's own stored description; **no data is ever inlined into an onclick attribute** (fixes an earlier bug where apostrophes/quotes in AI copy broke the single-quoted onclick / could inject markup). Tests: `{,tournament.,loyalty.,wheel.}offerTerms.test.js` + `api.{campaign,tournament,loyalty,wheel}.description.test.js` (771 total green). Verified live (campaign DE/UK, tournament BR-prize_drop, wheel VIP; loyalty is guest-gated → covered by integration test). **Same change also made Step-4 AI generation button-triggered instead of auto** (Bonus texts/audit no longer auto-fire on step entry; Tournament **Games** gated behind a "Recommend games" button in both forks — was auto-fetched on step-3 entry; Loyalty/Wheel were already button-driven).
- ~~**Wheel of Fortune — Generator hub tab (region-tuned wizard)**~~ — done 2026-07-20. New 🎡 Wheel tab in the unified Generator hub (`generator.html`), alongside Bonus/Tournament/Loyalty. New fork `public/generator-wheel.js` (globals prefixed `wh*`, mirrors the tournament/loyalty fork pattern: `wh-root`/`wh-content` container, `whInit`/`whShowView`/`whSetLang`/`whToggleGlossary`, lazy-init from `genSwitchType`). Flow: **library (saved wheels) → setup wizard → result**. Setup is **region-tuned** — `whRegionRecommend(geo, segment)` auto-picks preset + cadence with a rationale note (VIP→weekly vip-wheel; dormant→daily reactivation; 🇧🇷 BR→daily, flags welcome-bonus prohibition; else welcome/on_deposit acquisition); changing geo/segment re-applies it. Result: SVG wheel + segment legend + 6 econ cards (client `window._wheelEcon` + server `/api/wheel/generate`) + **AI CRM texts** (`/api/wheel/texts`, channel tabs push/email/sms/telegram/popup) + **compliance audit** (`/api/wheel/audit`) + Save (`savedWheels` localStorage + `configs` mirror) + Add-to-Calendar (`rc_campaigns` + `calendar-events` mirror). Wired in `generator.js` (`genSwitchType`/`genSetLang`/`genToggleGlossary` + `whInitialized`), `generator.html` (type-switch button, `wh-root`, `#wh-root` scoped CSS block, `wheel-econ.js`+`generator-wheel.js` script tags). Also fixed the Configurator wheel segment table (money prizes edited in raw site currency — no display-factor division that zeroed crypto/high-divisor amounts; inline unit suffix; fixed `table-layout` colgroup). Verified live (hub + region generate BR/DE).
- ~~**Wheel of Fortune — calculator + generator (first iteration)**~~ — done 2026-07-19, from expert feedback (full retention packages want welcome/reload/cashback/loyalty **+ gamification: tournaments/missions/wheel**). New 4th promo type in the unified Configurator (`configurator.js`: `CS.wheel`, type tab, `renderWheelSection` — SVG wheel visual + segment editor table + econ cards + scenario table + AI Audit/Optimize/Games tabs, i18n EN+RU). **First variant = fixed presets** (welcome / daily / VIP — user tweaks each segment's weight + prize value but can't add/remove segments; full editor is roadmapped, see Pending P2). Backend `src/domain/wheel/` (`presets.ts`, `buildWheel.ts` — `segmentCost` routes bonus_money/multiplier through the bonus engine's `truncNormalPayout` so wheel EV is consistent with bonus cost; free_spins = spins×bet×RTP, cashback = fraction×deposit; `calcEconomics.ts` — EV/spin, program cost, gamification GGR uplift + retention value, ROI, cost ratio, max risk, break-even, low/mid/high participation scenarios, reuses tournament's `deriveLocalFxRate`+`ARPU_BY_REGION`+segment ratios). Full stack: `wheel.service.ts`, `GenerateWheel.ts`, `wheel.controller.ts`, `wheel.routes.ts` (`/api/wheel/{generate,texts,audit,optimize}`, `optionalAuth`+`requireFeature('wheel')`), `wheel.schema.ts`, AI prompts (`wheel-{texts,audit,optimize}.prompt.ts`) + parsers. New `wheel` feature added to `FEATURES`/`GUEST_FEATURES` (open to guests, like tournaments). Client mirror `public/wheel-econ.js` (payout + presets + materialization + econ, parity-tested). Tests: `wheel.calcEconomics.test.js`, `wheel.econ.parity.test.js`, `integration/api.wheel.test.js` (712 total green). Verified end-to-end live (guest generate 200, static served, `/api/features` wheel:true, validation 400). Save writes to `cfgSaved` only (no calendar/badge/server-mirror yet — roadmapped).
