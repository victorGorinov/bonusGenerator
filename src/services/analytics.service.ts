...
export async function explainDivergence(comparison: CampaignComparison, ai: AIProvider): Promise<string> {
  const prompt = buildExplainPrompt(comparison);
  const explanation = await ai.generate(prompt, { maxTokens: 400 });
  return explanation;
}

function buildExplainPrompt(c: CampaignComparison): string {
  const costDeviation = c.costVariancePct >= 0 ? 'higher' : 'lower';
  const roi = c.roiActual >= 0 ? `positive (${c.roiActual.toFixed(0)}%)` : `negative (${c.roiActual.toFixed(0)}%)`;
