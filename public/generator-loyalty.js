...
function lySetTopbar(sub) {
  if (typeof GEN_TYPE !== 'undefined' && GEN_TYPE !== 'loyalty') return;
  const titleEl = document.getElementById('tb-title');
  const subEl   = document.getElementById('tb-sub');
  const sepEl   = document.getElementById('tb-sep');
  if (titleEl) titleEl.textContent = 'Loyalty Generator';
...
const L = {
  en: {
    ...
    save_btn:     'Save Program', calendar_btn: '📅 Add to Calendar', open_calendar: '📅 Open in Calendar',
  ...
  ru: {
    ...
    save_btn:     'Сохранить', calendar_btn: '📅 Добавить в календарь', open_calendar: '📅 Открыть в календаре',
...
function fmtMo(n) {
  if (n === undefined || n === null || !isFinite(n)) return lyT('never_breakeven');
...
function parseRecTarget(param, target) {
  ...
  switch (param) {
    ...
    case 'mode':            return /hybrid|tiers|missions/.exec(s)?.[0] ?? lyDraft.mode;
  ...
  if (isNaN(val)) return lyDraft[param];
...
function applyAiRecs(recs) {
  const beforeDraft = { ...lyDraft };
  const beforeEcon  = _lastData?.econ || null;
  for (const r of (recs || [])) {
    if (r.param in lyDraft) lyDraft[r.param] = parseRecTarget(r.param, r.target);
  }
...
function balanceToProfit(targetRoi) {
  ...
  const local = _recalcLocal(lyDraft);
  if (!local) { lyShowToast('Client econ not loaded'); return; }
  ...
  while (econ.roi3m < targetRoi && guard++ < 60) {
    ...
    for (const L of LEVERS) {
      ...
      if (Math.abs(val - cur) > 1e-9) { lyDraft[L.p] = val; moved = true; break; }
    }
    if (!moved) break;
    econ = (_recalcLocal(lyDraft) || {}).econ || econ;
  }

  if (econ.roi3m < targetRoi) lyShowToast(lyT('cannot_reach_target'));
  finishApply(beforeDraft, beforeEcon);
}

async function finishApply(beforeDraft, beforeEcon) {
  _undoStack = { lyDraft: beforeDraft, econ: beforeEcon };
  ...
  try {
    ...
    const missionSection = missions.length > 0
      ? `<div class="card"><div class="card-title">🎯 ${lyT('missions_lbl')} (${missions.length})</div>${missionListHTML(missions)}</div>` : '';
    ...
    _showChangedParams(beforeDraft, lyDraft);
  } catch(e) {
    if (body) body.innerHTML = `<div class="alert alert-warn">Error: ${lyEsc(e.message)}</div>`;
  }
}
...
function _showChangedParams(before, after) {
  ...
  for (const key of Object.keys(UI_BOUNDS).concat(['numTiers', 'mode'])) {
    ...
    changes.push(`<strong>${lyEsc(label)}</strong>: ${fmtV(bv)} → ${fmtV(av)}`);
  }
  ...
  el.innerHTML = `${lyT('changed_params')} ${changes.join(' · ')}`;
...
function undoApply() {
  ...
  Object.assign(lyDraft, _undoStack.lyDraft);
...
function actionPanelHTML(econ) {
  ...
  return `<div class="action-panel">
    <div class="roi-slider-wrap">
      <span class="roi-slider-label">${lyT('target_roi')}:</span>
    ...
    <button class="btn btn-sm btn-outline" ${hasOpt ? '' : 'disabled title="' + lyT('apply_recs_hint') + '"'}
    ...
    ${_undoStack ? `<button class="btn btn-sm btn-ghost" onclick="undoApply()">${lyT('undo')}</button>` : ''}
  </div>`;
}
...
function missionLinkHTML(link) {
  ...
  const accelBadge = link.acceleratesUpgrade
    ? `<span class="mission-tag" style="color:#10b981">⬆ ${lyEsc(lyT('mission_accelerates'))}</span>`
    : '';
  return `<div class="mission-link-block">
    <div class="mission-link-row">
      <span class="mission-tag">⬆ ${link.monthlyTierPoints} ${lyEsc(lyT('mission_tier_points'))}</span>
      ${accelBadge}
    </div>
    <div class="mission-link-row">
      <span class="mission-tag" style="color:#a0b0ff">⚡ ${lyEsc(lyT('mission_boost', boostFmt, link.boostDurationDays))}</span>
    </div>
    <div class="mission-link-row" style="font-size:.72rem;color:var(--muted)">
      📊 ${lyEsc(lyT('mission_eligible'))} ${lyEsc(tierLabels)}
    </div>
...
function missionListHTML(missions) {
  ...
  return missions.map(m => {
    ...
    const narrative = m.narrative
      ? `<div class="mission-narrative">💬 ${lyEsc(m.narrative)}</div>`
      : '';
    return `<div class="mission-row${linkBlock ? ' has-link' : ''}">
      <div class="mission-name">${icon} ${lyEsc(m.name)}</div>
      <div class="mission-meta">
        <span class="mission-tag">${lyEsc(freq)}</span>
        <span>${lyT('mission_target')} ${m.target} ${m.objective.replace('_', ' ')}</span>
        <span>${lyT('mission_reward')} <strong>${reward}</strong></span>
      </div>
      ${linkBlock}
      ${narrative}
    </div>`;
...
function render() {
  const el = document.getElementById('ly-content');
  if (!el) return;
  if (_view === 'list')   { renderListView(); return; }
  if (_view === 'setup')  { renderSetupView(); return; }
  if (_view === 'detail') { renderDetailView(lyDetailId); return; }
}

// ── LIST VIEW ─────────────────────────────────────────────────────────────────

function renderListView() {
  lySetTopbar(lyT('topbar_list'));
...
<div class="step-header">
  <div class="step-badge">⭐ ${lyT('list_heading')}</div>
  <div class="step-title">${lyT('list_lib_title')}</div>
  <div class="step-sub">${lyT('list_empty')}</div>
</div>
<div class="card" style="text-align:center;padding:40px 20px">
  <div style="font-size:2.5rem;margin-bottom:14px">⭐</div>
  <div style="color:var(--muted);font-size:.88rem;margin-bottom:20px">${lyT('list_empty_sub')}</div>
  <button class="btn btn-primary" onclick="lyShowView('setup')">${lyT('new_program')}</button>
</div>`;
  } else {
    const rows = programs.map(p => {
      ...
      const modeLabel = { tiers: lyT('mode_tiers'), missions: lyT('mode_missions'), hybrid: lyT('mode_hybrid') };
      const mode     = modeLabel[cfg?.mode] || '—';
      const tiers    = cfg?.tiers?.length ?? '—';
      const date     = p.createdAt ? new Date(p.createdAt).toLocaleDateString() : '';
      return `<div class="ct-row" onclick="lyShowView('detail','${lyEsc(p.id)}')" oncontextmenu="openMenu(event,'${lyEsc(p.id)}');return false">
        <div>
          <div class="ct-name">${lyEsc(p.name)}</div>
          <div class="ct-meta">${date}</div>
        </div>
        <div class="ct-cell">${lyEsc(mode)}</div>
        <div class="ct-cell">${lyEsc(String(tiers))}</div>
        <div class="ct-cell">${lyEsc(lift)}</div>
        <div class="ct-cell">${lyEsc(roi)}</div>
        <div class="ct-cell">
          <button class="btn btn-ghost btn-sm" style="padding:3px 6px" onclick="openMenu(event,'${lyEsc(p.id)}');event.stopPropagation()">⋯</button>
        </div>
      </div>`;
    }).join('');
...
<div class="step-header">
  <div class="step-badge">⭐ ${lyT('list_heading')}</div>
  <div class="step-title">${lyT('list_lib_title')}</div>
  <div class="step-sub">${lyT('list_count', programs.length)}</div>
</div>
    <div class="ctable">
      <div class="ct-hd">
        <div>${lyT('list_name_hdr')}</div>
        <div>${lyT('list_mode')}</div>
        <div>${lyT('list_tiers')}</div>
        <div>${lyT('list_lift')}</div>
        <div>${lyT('list_roi')}</div>
        <div></div>
      </div>
      ${rows}
    </div>
    <div style="margin-top:16px;text-align:center">
      <button class="btn btn-primary" onclick="step=1;lyShowView('setup')">${lyT('new_program')}</button>
    </div>`;
  }

  document.getElementById('ly-content').innerHTML = body;
}
...
function wizProgressHTML(current) {
  const labels = [lyT('step1_title'), lyT('step2_title'), lyT('step3_title')];
...
function renderStep1() {
  lySetTopbar(lyT('step1_topbar', lyT('step1_title')));

  const modeCards = MODES.map(m => `
    <div class="chip type-card ${lyDraft.mode === m.val ? 'on' : ''}" onclick="setDraft('mode','${m.val}');renderStep1()">
  ...
  document.getElementById('ly-content').innerHTML = `
  ${wizProgressHTML(1)}
  <div class="step-header">
    <div class="step-title">${lyT('step1_title')}</div>
    <div class="step-sub">${lyT('step1_sub')}</div>
  </div>

  <div class="card">
    <div class="card-title">${lyT('mode')}</div>
  ...
  <div class="card">
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px">
      <div class="form-row" style="margin-bottom:0">
        <label class="form-label">${lyT('players')}</label>
        <input class="form-input" type="number" min="100" max="500000" value="${lyDraft.players}"
  ...
  <div class="nav-footer">
    <button class="btn btn-ghost" onclick="lyShowView('list')">${lyT('back')}</button>
    <button class="btn btn-primary" onclick="lyGoStep(2)">${lyT('next')}</button>
  </div>`;
}

function renderStep2() {
  lySetTopbar(lyT('step2_topbar', lyT('step2_title')));

  const tierChips = [3, 4, 5].map(n => `
    <div class="chip ${lyDraft.numTiers === n ? 'on' : ''}" onclick="setDraft('numTiers',${n});renderStep2()">${lyT('tier_count_chip', n)}</div>
  `).join('');

  const cbPct = Math.round(lyDraft.topCashbackRate * 100);

  const expiryOpts = [
    [0, lyT('expiry_never')], [3, '3 mo'], [6, '6 mo'], [12, '12 mo'], [18, '18 mo'], [24, '24 mo'],
  ].map(([v, l]) =>
    `<option value="${v}" ${lyDraft.pointsExpiry === v ? 'selected' : ''}>${l}</option>`
  ).join('');

  const preview = calcTiersPreview(lyDraft);

  const missionRow = lyDraft.mode !== 'tiers' ? `
    <div class="form-row">
      <label class="form-label">${lyT('missions')} <span style="color:var(--accent)">(${lyDraft.missionCount})</span></label>
      <div class="range-wrap">
        <input type="range" min="0" max="6" value="${lyDraft.missionCount}"
          oninput="setDraft('missionCount',+this.value);document.getElementById('mc-val').textContent=this.value">
        <span class="range-val" id="mc-val">${lyDraft.missionCount}</span>
      </div>
    </div>` : '';
  ...
  ${wizProgressHTML(2)}
  <div class="step-header">
    <div class="step-title">${lyT('step2_title')}</div>
    <div class="step-sub">${lyT('step2_sub')}</div>
  </div>

  <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;align-items:start">
    <div>
      <div class="card">
        <div class="card-title">${lyT('tiers_lbl')}</div>
        <div class="form-row">
          <label class="form-label">${lyT('num_tiers')}</label>
          <div class="chips">${tierChips}</div>
        </div>
        <div class="form-row">
          <label class="form-label">${lyT('top_cashback')} <span style="color:var(--accent)">${cbPct}%</span></label>
          <div class="range-wrap">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          <div class="form-row" style="margin-bottom:0">
            <label class="form-label">${lyT('earn_dep')}</label>
            <input class="form-input" type="number" min="1" max="50" value="${lyDraft.earnRateDeposit}"
              oninput="setDraft('earnRateDeposit',+this.value);updateTierPreview()">
          </div>
          <div class="form-row" style="margin-bottom:0">
            <label class="form-label">${lyT('earn_wag')}</label>
            <input class="form-input" type="number" min="0" max="10" step="0.1" value="${lyDraft.earnRateWager}"
      ...
      <div class="card">
        <div class="card-title">${lyT('redeem_expiry_title')}</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          <div class="form-row" style="margin-bottom:0">
            <label class="form-label">${lyT('redeem_rate')}</label>
            <input class="form-input" type="number" min="10" max="1000" value="${lyDraft.redeemRate}"
              oninput="setDraft('redeemRate',+this.value)">
          </div>
          <div class="form-row" style="margin-bottom:0">
            <label class="form-label">${lyT('redeem_min')}</label>
            <input class="form-input" type="number" min="0" max="10000" step="100" value="${lyDraft.redeemMinPoints}"
              oninput="setDraft('redeemMinPoints',+this.value)">
          </div>
        </div>
        <div class="form-row" style="margin-top:10px;margin-bottom:0">
          <label class="form-label">${lyT('expiry')}</label>
          <select class="form-input" onchange="setDraft('pointsExpiry',+this.value)">${expiryOpts}</select>
        </div>
      </div>

      ${lyDraft.mode !== 'tiers' ? `<div class="card">
        <div class="card-title">${lyT('missions_lbl')}</div>
        ${missionRow}
      </div>` : ''}
    </div>

    <div class="card" style="position:sticky;top:70px">
      <div class="card-title">🔍 ${lyT('tier_preview')}</div>
      <div id="tier-preview">${tierTableHTML(preview)}</div>
      <div style="margin-top:12px;font-size:.73rem;color:var(--muted)">
        ${lyT('pts_formula', lyDraft.avgdep, lyDraft.earnRateDeposit)}
      </div>
    </div>
  </div>

  <div class="nav-footer">
    <button class="btn btn-ghost" onclick="lyGoStep(1)">${lyT('back')}</button>
    <button class="btn btn-primary" onclick="lyGoStep(3)">${lyT('gen_btn')}</button>
  </div>`;
}
...
function updateTierPreview() {
  ...
  if (!el) return;
  el.innerHTML = tierTableHTML(calcTiersPreview(lyDraft));
}
...
async function generateProgram() {
  ...
  lySetTopbar(isRu ? 'Генерация…' : 'Generating…');

  const c = document.getElementById('ly-content');
  c.innerHTML = `
    <div class="prog-wrap">
        ...
        setTimeout(() => {
          if (lastResult?._error) {
            c.innerHTML = `<div class="alert alert-warn" style="max-width:480px;margin:40px auto">
              Error: ${lyEsc(lastResult._error)}
              <button class="btn btn-outline" style="margin-top:10px;display:block" onclick="lyGoStep(2)">${lyT('back')}</button>
            </div>`;
          } else {
...
let _undoStack   = null;  // { lyDraft, econ } snapshot for Undo (1 step)
let _prevEcon    = null;  // econ before last apply — shown as delta

function renderStep3(data) {
  lySetTopbar(lyT('step3_topbar', lyT('step3_title')));
  ...
  const missionSection = missions.length > 0 ? `
    <div class="card">
      <div class="card-title">🎯 ${lyT('missions_lbl')} (${missions.length})</div>
  ...
  ${wizProgressHTML(3)}
  <div class="step-header">
    <div class="step-title">${lyT('step3_title')}</div>
    <div class="step-sub">${lyT('step3_sub')}</div>
  </div>
  ...
  <div class="nav-footer">
    <button class="btn btn-ghost" onclick="lyGoStep(2)">${lyT('back')}</button>
    <div style="display:flex;gap:8px">
      <button class="btn btn-outline" onclick="saveCurrentProgram()">${lyT('save_btn')}</button>
    </div>
  </div>`;
}
...
function renderAiTabBody(data, tiers, missions, missionSection) {
  ...
  if (_aiTab === 'optimize') {
    if (!_aiOpt) return renderAiFetchPrompt('optimize', lyT('opt_fetch_optimize'));
    return renderOptimizeHTML(_aiOpt);
  }
  if (_aiTab === 'missions') {
    if (!_aiMissions) return renderAiFetchPrompt('missions', lyT('opt_fetch_missions'));
...
function renderMissionsNarrativeHTML(missions) {
  ...
  return missions.map(m => {
    const icon = MISSION_ICONS[m.objective] || '🎯';
    const narrative = m.narrative ? `<div class="mission-narrative">💬 ${lyEsc(m.narrative)}</div>` : '';
    const tierEffect = m.tierEffect ? `<div style="font-size:.72rem;color:#a0b0ff;margin-top:4px">⚡ ${lyEsc(m.tierEffect)}</div>` : '';
    return `<div class="card" style="margin-bottom:10px">
      <div style="font-weight:700;margin-bottom:6px">${icon} ${lyEsc(m.name)}</div>
      ${narrative}
      ${tierEffect}
    </div>`;
  }).join('');
...
async function fetchAI(tabKey, detailId) {
  ...
  try {
    ...
    if (body) body.innerHTML = `<div class="alert alert-warn">Error: ${lyEsc(e.message)}
      <button class="btn btn-sm btn-outline" style="margin-left:10px" onclick="fetchAI(${retryArg})">${lyT('opt_retry')}</button></div>`;
  }
}
...
function renderTextsHTML(texts) {
  ...
  return channels.map(ch => {
    ...
    const rows = items.map((item, i) => {
      ...
      const sub   = body ? `<div style="font-size:.78rem;color:var(--muted);margin-top:3px">${lyEsc(body)}</div>` : '';
      const cta   = item.cta ? `<div style="font-size:.72rem;font-weight:700;color:#a0b0ff;margin-top:4px">CTA: ${lyEsc(item.cta)}</div>` : '';
      return `<div class="text-variant">
        <div class="text-variant-label">v${i + 1}</div>
        <div class="text-variant-body">${lyEsc(String(title))}</div>
        ${sub}${cta}
...
function renderAuditHTML(audit) {
  const checks = (audit.checks || []).map(c => `
    <div class="audit-check">
      ...
      <div>
        <div class="audit-label">${lyEsc(c.label)}</div>
        <div class="audit-note">${lyEsc(c.note)}</div>
        ${c.rule ? `<div style="font-size:.7rem;color:#a0b0ff;margin-top:3px">📋 ${lyEsc(c.rule)}</div>` : ''}
      </div>
    </div>`).join('');

  const recs = (audit.recommendations || []).map(r => `
    <div class="rec-card">
      <div class="rec-text">${lyEsc(r.text)}</div>
      <div class="rec-impact">${lyEsc(r.impact)}</div>
    </div>`).join('');
...
function renderOptimizeHTML(opt) {
  ...
  const impactLabel = { high: lyT('impact_high'), med: lyT('impact_med'), low: lyT('impact_low') };
  const recs = (opt.recommendations || []).map(r => {
    ...
    return `<div class="card" style="border-color:${ic}22;margin-bottom:10px">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px;margin-bottom:8px">
        <div style="font-weight:700;font-size:.88rem">${lyEsc(label)}</div>
        <span class="badge" style="background:${ic}20;color:${ic};white-space:nowrap">${lyEsc(impactLabel[r.impact] || r.impact)}</span>
      </div>
      <div style="display:flex;gap:10px;font-size:.8rem;margin-bottom:8px">
        <div style="color:var(--muted)">${lyT('opt_now')}: <strong style="color:var(--text)">${lyEsc(r.current)}</strong></div>
        <div style="color:var(--muted)">→ <strong style="color:${ic}">${lyEsc(r.target)}</strong></div>
      </div>
      <div style="font-size:.79rem;color:var(--muted);line-height:1.5">${lyEsc(r.reason)}</div>
    </div>`;
  }).join('');
...
function renderDetailView(id) {
  ...
  if (!p) { lyShowView('list'); return; }

  lySetTopbar(lyEsc(p.name));
  ...
  const missionTab = missions.length > 0
    ? `<button class="tab" data-tab="missions-detail" onclick="fetchAI('missions','${lyEsc(id)}')" style="margin-top:8px">${lyT('mission_describe_btn')}</button>`
    : '';
  ...
  document.getElementById('ly-content').innerHTML = `
  <div class="step-header">
    <div class="step-badge">${lyT('detail_lbl')} ${date}</div>
    <div class="step-title">${lyEsc(p.name)}</div>
  </div>
  ...
  <div class="nav-footer">
    <button class="btn btn-ghost" onclick="lyShowView('list')">${lyT('back')}</button>
    <div style="display:flex;gap:8px">
      <button class="btn btn-outline" onclick="deleteProgramById('${lyEsc(id)}')">🗑 Delete</button>
      <a class="btn btn-gold" href="/retention-calendar.html">${lyT('open_calendar')}</a>
    </div>
  </div>`;
}
...
function openMenu(e, id) {
  ...
  menu.innerHTML = `
    <button class="ly-menu-item" onclick="lyShowView('detail','${lyEsc(id)}')">📋 View</button>
    <div class="ly-menu-sep"></div>
    <button class="ly-menu-item" style="color:#ef4444" onclick="deleteProgramById('${lyEsc(id)}')">🗑 Delete</button>`;
  document.body.appendChild(menu);
  setTimeout(() => document.addEventListener('click', lyCloseMenu, { once: true }), 10);
}
function lyCloseMenu() {
...
function saveCurrentProgram() {
  if (!lastResult) return;
  const programs = loadPrograms();
  const name = autoName(lyDraft.mode, lyDraft.region, lyDraft.segment);
  const now  = new Date().toISOString();
  const entry = { id: genId(), name, result: lastResult, params: { ...lyDraft }, createdAt: now, updatedAt: now };
  programs.push(entry);
  savePrograms(programs);
  window.RetomatRepo?.mirror('loyalty-programs', entry.id, entry);
  // Merged action: saving also schedules the program in the Retention Calendar
  // (silent — no confirm/toast; dupes by region/segment/mode are skipped).
  try { _doAddToCalendar(_buildCalendarEntry(lastResult, lyDraft), { silent: true, savedId: entry.id }); } catch {}
  updateAllBadges();
  lyShowToast(lyT('saved_toast'));
}

function deleteProgramById(id) {
  if (!confirm(lyT('delete_confirm'))) return;
...
function _doAddToCalendar(campaign, opts = {}) {
  ...
  try {
    ...
    if (dupe) {
      ...
      if (!confirm(lyT('calendar_dupe', dupe.title, date))) return false;
    }
    const now = new Date().toISOString();
    ...
    lyShowToast(lyT('calendar_toast'));
  } catch { return false; }
  return true;
}
...
function lyAddDetailToCalendar(id) {
  const p = loadPrograms().find(x => x.id === id);
  if (!p) return;
...
function lyToggleLoyaltyGlossary() {
