...
function tgSetTopbar(sub) {
  if (typeof GEN_TYPE !== 'undefined' && GEN_TYPE !== 'tournament') return;
  const titleEl = document.getElementById('tb-title');
  const subEl   = document.getElementById('tb-sub');
  const sepEl   = document.getElementById('tb-sep');
  if (titleEl) titleEl.textContent = 'Tournament Generator';
  if (subEl)   subEl.textContent   = sub || '';
  if (sepEl)   sepEl.style.display = sub ? '' : 'none';
}

function tgSetTournLang(lang) {
  ...
  applyNavLang(lang);
  if (_tgCurrentView === 'list') tgShowView('list');
  else if (_tgCurrentView === 'detail') tgShowView('detail', tgDetailId);
  else if (_tgCurrentView === 'setup') tgShowView('setup');
  else if (tgHasActiveGenerator && tgStep > 0) tgRenderStep();
}
...
let tgStep            = 1;
let tgDetailId        = null;
let tgHasActiveGenerator = false;
...
let tgLastResult   = null;
let tgLastTexts    = null;
let tgLastAudit    = null;
let tgLastDesc     = null;
let tgLastOptimize = null;
let tgActiveTab    = 'push';
let tgActiveAudit  = false;
...
function tgAutoName(type, params) {
...
function _gamesParamsKey() {
  ...
  return `${p.geo}|${p.segment}|${tgDraft.type}|${p.scoring}`;
}
...
function renderGamesBlockHTML() {
  ...
  const labels = {
    ...
    aiNote:   isRu ? 'AI-предложение — проверьте соответствие вашему каталогу' : 'AI tgDraft — verify against your actual game catalog',
    rtp:      'RTP',
...
function tgWizProgressHTML(current) {
...
function tgGoStep(n) {
  tgStep = n;
  tgHasActiveGenerator = true;
  _tgCurrentView = 'generator';
  tgSetTopbar(tg('step_of', n));
  tgSetSidebarActive('nav-tournament');
  tgRenderStep();
}

function tgRenderStep() {
  const c = document.getElementById('tg-content');
  if      (tgStep === 1) c.innerHTML = tgRenderStep1();
  else if (tgStep === 2) c.innerHTML = tgRenderStep2();
  else if (tgStep === 3) c.innerHTML = tgRenderStep3();
  else if (tgStep === 4) c.innerHTML = tgRenderStep4();
  // Games are generated on demand (button in gamesSection) — no auto-fetch on step 3.
}

function tgSetSidebarActive(id) {
  document.querySelectorAll('.sb-nav .nav-item').forEach(el => el.classList.remove('active'));
  const el = document.getElementById(id);
  if (el) el.classList.add('active');
}

function tgShowSetupGuide() { tgShowView('setup'); }

function tgShowView(view, id) {
  const c  = document.getElementById('tg-content');
  _tgCurrentView = view;
  if (view === 'list') {
    tgSetTopbar(tg('topbar_list'));
    const fromTournGen = new URLSearchParams(location.search).get('view') === 'list';
    tgSetSidebarActive(fromTournGen ? 'nav-tourn-gen' : 'nav-tournament');
    c.innerHTML = renderList();
  } else if (view === 'detail') {
    tgDetailId = id;
    const t = tgLoadTournaments().find(t => t.id === id);
    tgSetTopbar(t ? t.name : tg('topbar_list'));
    tgSetSidebarActive('nav-tourn-gen');
    c.innerHTML = renderDetail(id);
  } else if (view === 'setup' || view === 'generator') {
    if (view === 'setup') {
      tgSetTopbar(tg('topbar_setup'));
      tgSetSidebarActive('nav-setup-guide');
      c.innerHTML = renderSetupGuide();
    } else {
      if (!tgDraft.type) tgDraft.type = 'slot';
      if (!tgDraft.params) tgDraft.params = { segment: 'all', totalPlayers: 5000, currency: 'EUR' };
      if (!tgDraft.params.currency) tgDraft.params.currency = GEO_TO_CUR_UI[tgDraft.params.geo || 'de'] || 'EUR';
      if (tgStep === 0) tgStep = 1;
      tgSetTopbar(tg('topbar_gen'));
      tgSetSidebarActive('nav-tourn-gen');
      tgRenderStep();
    }
  }
...
const GEO_FX_RATE_UI = {
  ...
  iq:1310, ly:4.85, sy:13000, ae:3.6725,
};
...
function updateEligibleHint() {
  const tp       = tgDraft.params.totalPlayers || 5000;
  const seg      = tgDraft.params.segment || 'all';
...
<div class="chips" style="gap:12px;margin-bottom:28px">
  ${TOURNAMENT_TYPES.map(t => `
    <div class="chip type-card${tgDraft.type===t.val?' on':''}" onclick="tgDraft.type='${t.val}';tgRenderStep()">
...
<div class="nav-footer">
  <span></span>
  <button class="btn btn-primary btn-lg" onclick="tgGoStep(2)">${tg('s1_next')}</button>
</div>`;
}

// ── STEP 2: Parameters ───────────────────────────────────────────────────────
function tgRenderStep2() {
  const p = tgDraft.params;
  const isRu = (localStorage.getItem('bonusLang') || 'en') === 'ru';
...
<div class="card">
  ...
  <div class="form-row">
    ...
    <div class="chips">
      ${TG_SEGMENTS.map(s => `<div class="chip${p.segment===s.val?' on':''}" onclick="tgDraft.params.segment='${s.val}';tgDraft.params.totalPlayers=tgDraft.params.totalPlayers||5000;tgRenderStep();updateEligibleHint()">${segLabels[s.val] || s.lbl}</div>`).join('')}
    </div>
  </div>
  <div class="form-row">
    ...
    <div style="display:flex;align-items:center;gap:12px">
      <input type="range" min="100" max="100000" step="100" id="f-tp"
             ...
             oninput="tgDraft.params.totalPlayers=+this.value;document.getElementById('tp-out').textContent=Number(+this.value).toLocaleString();updateEligibleHint();updatePrizeHint()"
...
<div class="card">
  ...
  <div class="form-row">
    ...
    <div class="chips">
      ${ENTRY_MODELS.map(e => `<div class="chip${p.entryModel===e.val?' on':''}" onclick="tgDraft.params.entryModel='${e.val}';tgRenderStep()">${entryLabels[e.val] || e.lbl}</div>`).join('')}
    </div>
  </div>
  <div class="form-row">
    <label class="form-label">${tg('scoring_lbl')}</label>
    <div class="chips">
      ${SCORING.map(s => `<div class="chip${p.scoring===s.val?' on':''}" onclick="tgDraft.params.scoring='${s.val}';tgRenderStep()">${scoringLbls[s.val] || s.lbl}</div>`).join('')}
    </div>
  </div>
  <div class="form-row">
    <label class="form-label">${tg('reentry_lbl')}</label>
    <div class="chips">
      ${REENTRY.map(r => `<div class="chip${p.reentry===r.val?' on':''}" onclick="tgDraft.params.reentry='${r.val}';tgRenderStep()">${reentryLbls[r.val] || r.lbl}</div>`).join('')}
    </div>
  </div>
</div>
...
<div class="card">
  ...
  <div class="form-row">
    ...
    <div class="chips">
      ${DURATIONS.map(d => `<div class="chip${p.duration===d.val?' on':''}" onclick="tgDraft.params.duration='${d.val}';tgRenderStep()">${durLabels[d.val] || d.lbl}</div>`).join('')}
    </div>
  </div>
  <div class="form-row">
    <label class="form-label">${tg('prize_lbl')}</label>
    <input class="form-input" type="number" min="100" id="f-pp" value="${p.prizePool}"
           onchange="tgDraft.params._prizeAutoSet=false;tgDraft.params.prizePool=Math.max(100,+this.value);tgRenderStep()">
  ...
  <div class="form-row">
    ...
    <div class="chips">
      ${POOL_MODELS.map(m => `<div class="chip${p.poolModel===m.val?' on':''}" onclick="tgDraft.params.poolModel='${m.val}';tgRenderStep()" title="${poolDescs[m.val] || m.desc}">${poolLabels[m.val] || m.lbl}</div>`).join('')}
    </div>
  </div>
  ${p.poolModel==='dynamic'?`
  <div class="form-row">
    <label class="form-label">${tg('rake_lbl')}</label>
    <input class="form-input" type="number" min="0" max="40" value="${p.rake||10}" onchange="tgDraft.params.rake=+this.value" style="max-width:120px">
  </div>`:''}
  <div class="form-row">
    <label class="form-label">${tg('distribution')}</label>
    <div class="chips">
      ${DISTRIBUTIONS.map(d => `<div class="chip${p.distribution===d.val?' on':''}" onclick="tgDraft.params.distribution='${d.val}';tgRenderStep()">${distLabels[d.val] || d.lbl}</div>`).join('')}
    </div>
  </div>
</div>
...
<div class="card">
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
    <div class="form-row" style="margin:0">
      ...
      <select class="form-input" onchange="tgDraft.params.lang=this.value">
    ...
    <div class="form-row" style="margin:0">
      ...
      <select class="form-input" onchange="tgDraft.params.tone=this.value">
...
<div class="nav-footer">
  <button class="btn btn-outline" onclick="tgGoStep(1)">${tg('btn_back')}</button>
  <button class="btn btn-primary btn-lg" id="btn-generate" onclick="tgRunGenerate()">${tg('btn_generate')}</button>
</div>`;
}

// ── TECH SPEC BUILDER ────────────────────────────────────────────────────────
function buildTechSpec() {
  if (!tgLastResult) return '';
  const r    = tgLastResult;
  const spec = r.spec;
  const cur  = r.cur || 'EUR';
  const p    = tgDraft.params;
  const isRu = (localStorage.getItem('bonusLang') || 'en') === 'ru';
...
function tgRenderStep3() {
  if (!tgLastResult) { tgGoStep(2); return ''; }
  const r    = tgLastResult;
  const e    = r.econ;
  const spec = r.spec;
  const cur  = r.cur || 'EUR';
  ...
  const dur = tgDraft.params.duration || 'weekly';
...
<div class="card">
  ...
  <div style="font-size:.7rem;color:var(--muted);margin-top:10px">
    ${tg('eligible_note', e.eligible, tgDraft.params.segment, Math.round(e.segmentRatio*100), (tgDraft.params.totalPlayers||5000).toLocaleString(), e.arpu, engMul.toFixed(1))}
  </div>
  <div style="margin-top:14px;display:flex;gap:10px;align-items:center">
    <button class="btn btn-outline btn-sm" onclick="tgRunOptimize()" id="btn-optimize">
      ${(localStorage.getItem('bonusLang')||'en')==='ru' ? '🤖 AI-ревью прогноза' : '🤖 AI Review'}
    </button>
  </div>
</div>

<div id="optimize-area">${tgLastOptimize ? tgRenderOptimizeHTML(tgLastOptimize, (roi<0||e.netMarginMid<0)?'optimize':'review') : ''}</div>
...
<div class="nav-footer">
  <div style="display:flex;gap:9px;align-items:center">
    <button class="btn btn-outline" onclick="tgGoStep(2)">${tg('btn_reconfig')}</button>
    <button id="btn-save-tournament" class="btn btn-outline"
            style="border-color:rgba(79,110,247,.4);color:#a0b0ff"
            onclick="tgSaveTournament()">
      ${tg('btn_save')}
    </button>
  </div>
  <div style="display:flex;gap:9px">
    <button class="btn btn-outline btn-sm" onclick="tgExportTournamentPDF()">${tg('btn_pdf')}</button>
    <button class="btn btn-primary btn-lg" onclick="tgGoStep(4)">${tg('btn_ai_texts')}</button>
  </div>
</div>`;
}
...
function tgRenderStep4() {
  ...
  const hasAudit = !!tgLastAudit;
  const hasDesc  = !!tgLastDesc;
...
<div style="display:flex;gap:10px;margin-bottom:20px;flex-wrap:wrap">
  <button class="btn btn-primary" onclick="tgRunTexts()" id="btn-texts">${hasTexts ? tg('btn_regen_texts') : tg('btn_gen_texts')}</button>
  <button class="btn btn-outline" onclick="tgRunDescription()" id="btn-desc">${hasDesc ? tg('btn_regen_desc') : tg('btn_gen_desc')}</button>
  <button class="btn btn-outline" onclick="tgRunAudit()" id="btn-audit">${hasAudit ? tg('btn_reaudit') : tg('btn_audit_lbl')}</button>
</div>

<div id="texts-area">${hasTexts ? tgRenderTextsHTML(tgLastTexts) : ''}</div>
<div id="desc-area">${hasDesc ? tgRenderTournDescHTML(tgLastDesc) : ''}</div>
...
<div class="nav-footer">
  ...
  <button class="btn btn-ghost" onclick="tgGoStep(1)">${tg('btn_start_over')}</button>
</div>`;
}

function tgRenderTextsHTML(texts) {
  const channels = ['push','email','sms','telegram','popup'];
  const tabs = channels.map(ch => `<button class="tab${tgActiveTab===ch?' active':''}" onclick="tgActiveTab='${ch}';document.getElementById('texts-area').innerHTML=tgRenderTextsHTML(tgLastTexts)">${ch.charAt(0).toUpperCase()+ch.slice(1)}</button>`).join('');

  let body = '';
  const variants = texts[tgActiveTab] || [];
  if (tgActiveTab === 'email') {
...
function tgRenderAuditHTML(audit) {
...
// ── PDF EXPORT ────────────────────────────────────────────────────────────────
function tgExportTournamentPDF() {
  if (!tgLastResult) return;
  const r    = tgLastResult;
  const spec = r.spec || {};
  const e    = r.econ || {};
...
function tgRunGenerate() {
  ...
  tgStep = 0; tgHasActiveGenerator = true;
  tgSetTopbar(isRu ? 'Генерация…' : 'Generating…');
  ...
  const apiPromise = fetch('/api/tournament/generate', {
    method:  'POST',
    headers: { 'Content-Type': 'application/json' },
    body:    JSON.stringify({ type: tgDraft.type, params: tgDraft.params }),
  })
  .then(r => r.ok ? r.json() : r.json().then(e => Promise.reject(new Error(e.message || r.statusText))))
  .then(data => { tgLastResult = data; tgLastTexts = null; tgLastAudit = null; tgLastDesc = null; tgLastOptimize = null; _tgUndoStack = null; _tgPrevEcon = null; _tgLastOptRecs = []; })
  .catch(err => { tgLastResult = { _error: err.message }; });
  ...
  (function tick() {
    ...
    if (i < steps.length) {
      ...
      Promise.resolve(apiPromise).then(() => {
        setTimeout(() => {
          if (tgLastResult?._error) {
            c.innerHTML = `<div class="alert alert-warn" style="max-width:480px;margin:40px auto">
              Error: ${tgLastResult._error}
              <button class="btn btn-outline btn-sm" style="margin-top:10px;display:block" onclick="tgGoStep(2)">← Back</button>
            </div>`;
          } else {
            tgGoStep(3);
          }
...
async function tgRunTexts() {
  ...
  try {
    const resp = await fetch('/api/tournament/texts', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ type: tgDraft.type, params: tgDraft.params, spec: tgLastResult?.spec || {} }),
    });
    if (!resp.ok) {
      const err = await resp.json().catch(() => ({ message: resp.statusText }));
      throw new Error(err.message || resp.statusText);
    }
    tgLastTexts = await resp.json();
    tgActiveTab = 'push';
    area.innerHTML = tgRenderTextsHTML(tgLastTexts);
...
async function tgRunAudit() {
  ...
  try {
    const resp = await fetch('/api/tournament/audit', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ type: tgDraft.type, params: tgDraft.params, spec: tgLastResult?.spec || {}, uiLang: tgDraft.params.lang }),
    });
    if (!resp.ok) {
      const err = await resp.json().catch(() => ({ message: resp.statusText }));
      throw new Error(err.message || resp.statusText);
    }
    tgLastAudit = await resp.json();
    area.innerHTML = tgRenderAuditHTML(tgLastAudit);
...
// ── OFFER DESCRIPTION ────────────────────────────────────────────────────────
async function tgRunDescription() {
  ...
  try {
    ...
    tgLastDesc = await resp.json();
    area.innerHTML = tgRenderTournDescHTML(tgLastDesc);
...
function tgRenderTournDescHTML(d) {
  return window.OfferDesc.render(d, { note: tg('desc_note'), hint: tg('desc_hint'), how: tg('desc_how'), tc: tg('desc_tc'), copy: tg('desc_copy'), copyFn: 'tgCopyDesc' });
}
function tgCopyDesc(btn) {
  if (!tgLastDesc) return;
  window.OfferDesc.copyText(window.OfferDesc.plainText(tgLastDesc, tg('desc_tc')), btn);
}

// ── AI OPTIMIZE / REVIEW ─────────────────────────────────────────────────────
async function tgRunOptimize() {
  const btn  = document.getElementById('btn-optimize');
  const area = document.getElementById('optimize-area');
  if (!btn || !area || !tgLastResult) return;

  const lang  = localStorage.getItem('bonusLang') || 'en';
  const isRu  = lang === 'ru';
  const e     = tgLastResult.econ;
  ...
  try {
    ...
    tgLastOptimize = await resp.json();
    _tgLastOptRecs = tgLastOptimize.recommendations || [];
    area.innerHTML = tgRenderOptimizeHTML(tgLastOptimize, mode);
    ...
    if (ap && tgLastResult) ap.outerHTML = tgActionPanelHTML(tgLastResult.econ);
  } catch (e) {
...
function applyTgAiRecs(recs) {
  if (!tgLastResult) return;
  const beforeParams = { ...tgDraft.params };
  const beforeEcon   = { ...tgLastResult.econ };

  for (const rec of recs) {
    if (rec.param && rec.target !== undefined && tgDraft.params[rec.param] !== undefined) {
      const val = parseTgRecTarget(rec.param, rec.target);
      tgDraft.params[rec.param] = val;
    }
  }
  tgDraft.params._prizeAutoSet = false;
...
async function balanceTgToProfit(targetRoi) {
  ...
  if (!tgLastResult) {
    tgShowToast(isRu ? 'Сначала сгенерируйте турнир' : 'Generate a tournament first');
    return;
  }
  if (!window._tournamentEcon || !window._balanceSolver) {
    tgShowToast(isRu ? 'Модули ещё загружаются, попробуйте ещё раз' : 'Modules still loading, try again');
    return;
  }

  // If already at target, nothing to do
  const currentRoi = tgLastResult.econ ? (tgLastResult.econ.roi ?? 0) : 0;
  ...
  try {
    const suggestedFloor = Math.round(calcSuggestedPrize(tgDraft.params).prize * 0.3);
    ...
    const LEVERS = [
      // Switch fixed→hybrid first: -40% prizePoolCost in one tgStep (hybrid always cheaper than dynamic at rake<40%)
    ...
    const { tgDraft: solvedParams, reached } = window._balanceSolver.solveToTarget({
      tgDraft:    { ...tgDraft.params },
      levers:   LEVERS,
    ...
    Object.assign(tgDraft.params, solvedParams);
    tgDraft.params._prizeAutoSet = false;

    if (!reached) {
      const msg = isRu
        ? `Не удалось достичь ROI ${targetRoi}% — упёрлись в минимальный приз (${prizeFloor.toLocaleString()} ${tgLastResult.cur || ''}). Применены лучшие параметры.`
        : `Could not reach ROI ${targetRoi}% — hit prize floor (${prizeFloor.toLocaleString()} ${tgLastResult.cur || ''}). Best parameters applied.`;
      tgShowToast(msg);
    }
    ...
    tgShowToast(isRu ? 'Ошибка при балансировке — см. консоль' : 'Balance error — see console');
  }
}
...
function undoTgApply() {
  ...
  tgDraft.params = { ..._tgUndoStack.params };
  tgLastResult   = tgLastResult ? { ...tgLastResult, econ: _tgUndoStack.econ } : null;
  _tgPrevEcon  = null;
  _tgUndoStack = null;
  _tgLastOptRecs = [];
  tgLastOptimize = null;
...
function tgRenderOptimizeHTML(data, mode) {
  const lang = localStorage.getItem('bonusLang') || 'en';
  const isRu = lang === 'ru';
...
<div class="card" style="text-align:center;padding:40px 20px">
  ...
  <button class="btn btn-primary" onclick="tgGoStep(1)">Create a Tournament →</button>
</div>`;
  }
...
<div class="nav-footer">
  <button class="btn btn-outline" onclick="tgGoStep(3)">← Back to Economics</button>
  <div style="display:flex;gap:9px">
    <button class="btn btn-outline btn-sm" onclick="tgExportTournamentPDF()">⬇ PDF</button>
    <button class="btn btn-primary" onclick="tgGoStep(4)">Generate AI Texts →</button>
  </div>
</div>`;
}

// ── ADD TO RETENTION CALENDAR ─────────────────────────────────────────────────
function tgAddTournamentToCalendar(opts = {}) {
  const silent = !!opts.silent; // silent = called from Save: no confirm, no toast, skip dupes
  if (!tgLastResult) return false;
  const { spec, econ, params, cur } = tgLastResult;
  ...
  const campaign = {
    title:      `${(tgDraft.type||'slot').charAt(0).toUpperCase() + (tgDraft.type||'slot').slice(1)} Tournament · ${(params?.geo||'').toUpperCase()}`,
    type:       'tournament',
...
// ── SAVE / DELETE ────────────────────────────────────────────────────────────
function tgSaveTournament() {
  if (!tgLastResult) return;
  const list = tgLoadTournaments();
  const existing = list.find(t =>
    t.type === tgDraft.type &&
    JSON.stringify(t.params) === JSON.stringify(tgDraft.params)
  );
  if (existing) { tgShowToast('Already saved'); return; }
  const entry = {
    id:        tgGenId(),
    name:      tgAutoName(tgDraft.type, tgDraft.params),
    type:      tgDraft.type,
    params:    { ...tgDraft.params },
    spec:      tgLastResult.spec,
    econ:      tgLastResult.econ,
    cur:       tgLastResult.cur,
    lic:       tgLastResult.lic,
    region:    tgLastResult.region,
    games:     (_gamesData && _gamesData._key === _gamesParamsKey()) ? _gamesData.result : null,
    createdAt: new Date().toISOString(),
  };
  list.unshift(entry);
  tgSaveTournaments(list);
  window.RetomatRepo?.mirror('tournaments', entry.id, entry);
  // Merged action: saving also schedules the tournament in the Retention Calendar
  // (silent — no confirm/toast; dupes by geo/segment/mechanic are skipped).
  try { tgAddTournamentToCalendar({ silent: true, savedId: entry.id }); } catch {}
  const btn = document.getElementById('btn-save-tournament');
  if (btn) { btn.textContent = '✓ Saved'; btn.disabled = true; btn.style.opacity = '.5'; }
  tgShowToast(tg('toast_saved'));
  updateNavBadge();
}

function tgDeleteTournament(id) {
  if (!confirm(tg('confirm_delete'))) return;
  tgSaveTournaments(tgLoadTournaments().filter(t => t.id !== id));
  window.RetomatRepo?.unmirror('tournaments', id);
  updateNavBadge();
  tgShowToast(tg('toast_deleted'));
  tgShowView('list');
}
...
function tournRowHTML(t) {
  ...
  return `<div class="ct-row" onclick="tgShowView('detail','${t.id}')">
    <div>
...
function showTournMenu(id, btn) {
  ...
  menu.innerHTML = `
    <button class="tourn-menu-item" onclick="tgShowView('detail','${id}')">↗ Details</button>
    <button class="tourn-menu-item" onclick="tgLoadAndShowGuide('${id}')">📋 Setup Guide</button>
    <button class="tourn-menu-item" onclick="tgLoadAndRegenTexts('${id}')">✦ AI Texts</button>
    <div class="tourn-menu-sep"></div>
    <button class="tourn-menu-item" onclick="tgDeleteTournament('${id}')" style="color:#f87171">🗑 Delete</button>`;
...
<div class="card" style="text-align:center;padding:40px 20px">
  ...
  <button class="btn btn-primary" onclick="tgGoStep(1)">${tg('list_create')}</button>
</div>`;
  }
...
<div style="margin-top:16px;text-align:center">
  <button class="btn btn-primary" onclick="tgGoStep(1)">${tg('list_new')}</button>
</div>`;
}
...
function renderDetail(id) {
  const t = tgLoadTournaments().find(t => t.id === id);
  if (!t) return `<div class="card">${tg('det_not_found')} <button class="btn btn-ghost" onclick="tgShowView('list')">${tg('det_back')}</button></div>`;
...
<div style="margin-bottom:18px">
  <button class="btn btn-ghost btn-sm" onclick="tgShowView('list')" style="padding:0;margin-bottom:10px;color:var(--muted);font-size:.8rem">${tg('det_back')}</button>
...
<div style="display:flex;gap:10px;margin-top:4px;flex-wrap:wrap">
  <button class="btn btn-outline" style="flex:1;border-color:rgba(124,58,237,.4);color:#c4b5fd" onclick="tgLoadAndShowGuide('${t.id}')">${tg('det_setup_guide')}</button>
  <button class="btn btn-outline" style="flex:1;border-color:rgba(79,110,247,.4);color:#a0b0ff" onclick="tgLoadAndRegenTexts('${t.id}')">${tg('det_ai_texts')}</button>
  <button class="btn btn-outline btn-sm" style="color:var(--muted);border-color:var(--border)" onclick="tgDeleteTournament('${t.id}')">${tg('det_delete')}</button>
</div>`;
}

// ── LOAD HELPERS ─────────────────────────────────────────────────────────────
function tgLoadAndShowGuide(id) {
  const t = tgLoadTournaments().find(t => t.id === id);
  if (!t) return;
  tgLastResult   = { spec: t.spec, econ: t.econ, cur: t.cur, lic: t.lic, region: t.region };
  tgDraft.type   = t.type;
  tgDraft.params = { ...t.params };
...
function tgLoadAndRegenTexts(id) {
  const t = tgLoadTournaments().find(t => t.id === id);
  if (!t) return;
  tgLastResult   = { spec: t.spec, econ: t.econ, cur: t.cur, lic: t.lic, region: t.region };
  tgLastTexts    = null;
...
function tgInit() {
  updateAllBadges();
  tgSetTournLang(localStorage.getItem('bonusLang') || 'en');
  resolveInitialTournamentView();
  window.addEventListener('pageshow', function() {
    if (GEN_TYPE === 'tournament') { updateAllBadges(); resolveInitialTournamentView(); }
  });
  ...
  window.addEventListener('retomat:synced', function() {
    if (GEN_TYPE === 'tournament') { updateAllBadges(); if (_tgCurrentView === 'list') tgShowView('list'); }
  });
}

function tgToggleTournGlossary() {
